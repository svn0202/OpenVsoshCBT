...
Для фиксации попыток переключения окна после базовой миграции наблюдения
примените `openvsosh_focus_monitoring.sql` из каталога своей СУБД. Миграция
добавляет серверный счётчик и идентификатор последнего принятого события.
