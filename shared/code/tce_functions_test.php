        ...
        if ($r = F_db_query($sql, $db)) {
            ...
            while ($m = F_db_fetch_array($r)) {
                ...
                if ($m['tsubset_type'] == 1) {
                    ...
                    $sqlq .= ' AND question_id IN (' . ($answers_order_questions_ids[$position_type] ?? '0') . ')';
                }
...
function F_updateQuestionLog($test_id, $testlog_id, $answpos = [], $answer_text = '', $reaction_time = 0)
{
    ...
    if ((int) $question_type === 5) {
        $answpos = f_normalize_matching_positions(is_array($answpos) ? $answpos : []);
    }
        ...
        if ($m = F_db_fetch_array($r)) {
            ...
            if ((int) $m['question_type'] === 5) {
                $str .=
                    ...
                    . json_encode($l['m_matching_position_reassigned'], JSON_THROW_ON_ERROR)
                    . ';}'
                    . '});});});'
                    . K_NEWLINE;
            }
