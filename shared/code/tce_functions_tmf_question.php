...
function f_tmf_question_options(string $description): array
{
    ...
    if (preg_match('/<!--TMF_MAX_SEL:(\d+)-->/', $description, $match)) {
        $options['max_selections'] = max(0, (int) ($match[1] ?? 0));
    ...
    if (preg_match('/<!--TMF_MCMA_HEADER:(.*?)-->/', $description, $match)) {
        ...
        $headers = $normalize_array(
            json_decode(html_entity_decode($match[1] ?? '', ENT_QUOTES, 'UTF-8'), true),
        );
        if ($headers !== null && count($headers) >= 4) {
    ...
    if (preg_match('/<!--TMF_SIMILARITY:(\d{1,3})-->/', $description, $match)) {
        $options['similarity_threshold'] = max(0, min(100, (int) ($match[1] ?? 0)));
    }
    if (preg_match('/<!--TMF_MATCH_POSITIONS:(\d{1,3})-->/', $description, $match)) {
        $options['matching_positions'] = max(0, min(100, (int) ($match[1] ?? 0)));
    }
    if (preg_match('/<!--TMF_AUDIO_PLAYS:(\d{1,2})-->/', $description, $match)) {
        $options['audio_play_limit'] = max(0, min(99, (int) ($match[1] ?? 0)));
    }
...
function f_tmf_matching_presentation(string $description, int $positions): array
{
    ...
    foreach (array_reverse($candidate_matches) as $candidate_capture) {
        ...
        $item_values = $items[1] ?? [];
        if (count($item_values) !== $positions) {
            continue;
        }
...
function f_tmf_text_similarity(string $left, string $right, bool $binary = false): float
{
    ...
    return round(100 * (1 - (($previous[$right_count] ?? 0) / $maximum)), 2);
}
...
{
    /** @var array<array-key, int|string> $answers */
    if ($maximum <= 0) {
        return true;
    }
