...
final class AdminUserEditorControllerTest extends TestCase
{
    ...
    public function testUpdatedUserPasswordConfirmationPreservesAllOutcomes(): void
    {
        [$status, $output] = \F_tcecode_run_process(
            [
                ...
                'namespace Harness; '
                    ...
                    . '$end = strpos($source, "\\n\\n                \\$sql =", $start); '
                    . '$block = substr($source, $start, $end - $start); '
                    . '$l = ["m_different_passwords" => "different"]; $results = []; '
                    . 'foreach ([["same", "same"], ["same", "other"], ["", ""]] as $case) {'
                    . '[$newpassword, $newpassword_repeat] = $case; $user_password = "stored-hash"; '
                    . '$user_otpkey = "stored-otp"; $formstatus = true; $GLOBALS["errors"] = []; '
                    . 'eval("namespace Harness; switch (1) { case 1: " . $block . " }"); '
                    . '$results[] = [$user_password, $user_otpkey, $formstatus, $GLOBALS["errors"]]; } '
                    . 'echo json_encode($results);',
                dirname(__DIR__) . '/admin/code/tce_edit_user.php',
        ...
        self::assertSame(
            [
                ['hash:same', 'new-otp', true, []],
                ['stored-hash', 'stored-otp', false, [['WARNING', 'different']]],
                ['stored-hash', 'stored-otp', true, []],
            ],
            json_decode($output, true, 512, JSON_THROW_ON_ERROR),
        );
    }
