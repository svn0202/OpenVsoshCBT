...
{
    public function testPopulatedXmlCreatesGroupUserAndMembershipInOrder(): void
    {
        [$status, $output] = \F_tcecode_run_process(
            [
                ...
                'define("K_TIMESTAMP_FORMAT", "Y-m-d H:i:s"); define("K_AUTH_ADMINISTRATOR", 10); '
                    ...
                    . '$GLOBALS["rows"] = [false, false, false]; $GLOBALS["ids"] = [7, 42]; '
                    ...
                    . '$class = substr($source, $start, $end - $start + strlen($marker)); '
                    . '$class = preg_replace("/^\\s*require_once [^;]+;\\n/m", "", $class); eval($class); '
                    . '$file = tempnam(sys_get_temp_dir(), "openvsosh-users-xml-"); '
                    . '$xml = "<users><user><name>alice</name><password>secret</password>" '
                    . '. "<email>alice@example.test</email><regdate>2024-01-02 03:04:05</regdate>" '
                    . '. "<ip>192.0.2.1</ip><firstname>Alice</firstname><lastname>Doe</lastname>" '
                    . '. "<birthdate>2000-01-01</birthdate><birthplace>Town</birthplace>" '
                    . '. "<regnumber>R-1</regnumber><ssn>S-1</ssn><level>3</level>" '
                    . '. "<verifycode>verify</verifycode><otpkey>otp</otpkey><group>Staff</group>" '
                    . '. "</user></users>"; file_put_contents($file, $xml); '
                    . '$importer = new XMLUserImporter($file); unset($importer); '
                    . 'echo json_encode([$GLOBALS["queries"], $GLOBALS["insert_calls"], '
                    . '$GLOBALS["errors"]]);',
        ...
        /** @var array{
         *   0: array{0: string, 1: string, 2: string, 3: string, 4: string, 5: string},
         *   1: list<array{0: string, 1: string}>,
         *   2: int
         * } $decoded
         */
        $decoded = json_decode($output, true, 512, JSON_THROW_ON_ERROR);
        [$queries, $insertCalls, $errors] = $decoded;
        self::assertSame(0, $errors);
        self::assertStringContainsString("WHERE group_name='Staff'", $queries[0]);
        self::assertStringContainsString('INSERT INTO groups', $queries[1]);
        self::assertStringContainsString("WHERE user_name='alice'", $queries[2]);
        self::assertStringContainsString("'hash:secret'", $queries[3]);
        self::assertStringContainsString("usrgrp_group_id='7'", $queries[4]);
        self::assertStringContainsString('42, 7', $queries[5]);
        self::assertSame([['groups', 'group_id'], ['users', 'user_id']], $insertCalls);
    }
